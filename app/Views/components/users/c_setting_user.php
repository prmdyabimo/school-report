    ...
    <div class="flex md:flex-row flex-col gap-4">
        ...
        <div class="flex flex-row md:flex-col items-center gap-4 p-4 shadow rounded-md lg:w-1/2 w-full h-full">
            <div class="flex flex-col items-center justify-center">
                <img class="w-36 h-36 rounded-md" src="<?= base_url('uploads/profile/' . $user['image']); ?>" alt="<?= $user['name']; ?>">
        ...
        <!-- RIGHT -->
        <form action="/user-profile/<?= $user['id']; ?>" method="post" class="form_edit flex flex-col gap-4 p-4 shadow rounded-md w-full">
            <?= csrf_field(); ?>
            <div class="flex md:flex-row flex-col w-full gap-4">
                ...
                <div class="relative w-full">
                    <input type="text" class="phone peer m-0 block h-[58px] w-full rounded border border-solid <?= (validation_show_error('email')) ? "border-red-500 text-red-500" : "border-neutral-300 text-neutral-700"; ?> bg-transparent bg-clip-padding px-3 py-4 text-base font-normal leading-tight text-neutral-800 transition duration-200 ease-linear placeholder:text-transparent focus:border-primary focus:pb-[0.625rem] focus:pt-[1.625rem] focus:text-neutral-800 focus:outline-none peer-focus:text-primary [&:not(:placeholder-shown)]:pb-[0.625rem] [&:not(:placeholder-shown)]:pt-[1.625rem]" name="email" placeholder="Masukkan email" value="<?= $user['email']; ?>" />
            ...
            <div class="flex md:flex-row flex-col w-full gap-4">
                <div class="relative w-full">
                    <input type="text" class="phone peer m-0 block h-[58px] w-full rounded border border-solid <?= (validation_show_error('phone')) ? "border-red-500 text-red-500" : "border-neutral-300 text-neutral-700"; ?> bg-transparent bg-clip-padding px-3 py-4 text-base font-normal leading-tight text-neutral-800 transition duration-200 ease-linear placeholder:text-transparent focus:border-primary focus:pb-[0.625rem] focus:pt-[1.625rem] focus:text-neutral-800 focus:outline-none peer-focus:text-primary [&:not(:placeholder-shown)]:pb-[0.625rem] [&:not(:placeholder-shown)]:pt-[1.625rem]" name="phone" placeholder="Masukkan phone" value="<?= $user['telephone']; ?>" />
...
<div data-te-modal-init class="fixed left-0 top-0 z-[1055] hidden h-full w-full overflow-y-auto overflow-x-hidden outline-none" id="changeImage" tabindex="-1" aria-labelledby="exampleModalCenterTitle" aria-modal="true" role="dialog">
    <div data-te-modal-dialog-ref class="pointer-events-none relative flex min-h-[calc(100%-1rem)] w-auto translate-y-[-50px] items-center opacity-0 transition-all duration-300 ease-in-out min-[576px]:mx-auto min-[576px]:mt-7 min-[576px]:min-h-[calc(100%-3.5rem)] min-[576px]:max-w-[500px]">
        <div class="pointer-events-auto relative flex w-full flex-col rounded-md border-none bg-white bg-clip-padding text-current shadow-lg outline-">
            ...
            <form class="form_edit" method="POST" action="/user-image/<?= $user['id']; ?>" enctype="multipart/form-data">
                <?= csrf_field(); ?>
                <input type="text" name="old_image" id="old_image" value="<?= $user['image']; ?>" hidden>

                <!--Modal body-->
                <div class="relative p-4">
                    <div class="relative w-full">
                        <img class="mx-auto" src="<?= base_url('uploads/profile/' . $user['image']); ?>" alt="<?= $user['name']; ?>">
                    </div>
